...
## Changelog
- 2026-03-04 hyp_006 STARTED. Output-Shift Multi-Molecule TarFlow. Hypothesis: the SOS+strictly-causal-mask autoregressive mechanism creates a pathway for log-det exploitation that doesn't exist in Apple's output-shift architecture. By switching to output-shift (cat([zeros, out[:,:-1]], dim=1)) with self-inclusive causal mask, we eliminate this pathway. The autoregressive structure is enforced by the output shift, not the mask — providing a HARD guarantee regardless of attention internals. Diagnostic gate: log_det/dof < 5.0 at step 500 with alpha_pos=10.0 and no regularization.
...
- **[REVISED — hyp_005]** Padding + attention mask was assumed to handle variable atom counts (9-21 atoms). und_001 showed padding catastrophically degrades TarFlow performance. hyp_005 tested two specific fixes (PAD token embedding, query zeroing) but they had zero effect — the failure is not padding corruption per se, but log-det exploitation in the SOS+causal architecture that is exacerbated in the multi-molecule setting. Per-molecule models (T=n_real, no padding) remain the viable path for TarFlow. The multi-molecule padding problem may require architectural changes (output-shift instead of SOS+causal).
...
- **[ACTIVE — hyp_006]** Padding is necessary but not sufficient to explain multi-molecule TarFlow failure. und_001 identified padding as the dominant factor (98% → 21% VF). hyp_005 showed that fixing the two padding corruption channels (PAD token embedding, query zeroing) has zero effect — the failure has a deeper root in the SOS+causal architecture's log-det gradient dynamics. hyp_006 tests the hypothesis that switching from SOS+strictly-causal to Apple's output-shift architecture eliminates log-det exploitation. In output-shift, the autoregressive structure is enforced by the shift (cat([zeros, out[:,:-1]])), not the mask — the mask serves internal computation only, making the Jacobian structure independent of masking correctness.
...
7. **hyp_006**: Output-Shift Multi-Molecule TarFlow — replace SOS+strictly-causal with Apple's output-shift mechanism in src/model.py. OPTIMIZE with diagnostic gate (log_det/dof bounded at 500 steps), then SANITY/HEURISTICS/SCALE on multi-molecule training. Success criterion: VF >= 0.5 on >= 4/8 molecules.
8. **hyp_007**: DDPM -- implement the diffusion baseline with comparable architecture, train and tune (OPTIMIZE).
9. **hyp_008** (conditional): Head-to-head comparison -- TarFlow vs DDPM on all metrics.
