...
- **[REVISED — hyp_006]** Padding + attention mask was assumed to handle variable atom counts (9-21 atoms). und_001 showed padding catastrophically degrades TarFlow performance. hyp_005 showed that fixing the two padding corruption channels (PAD token, query zeroing) has zero effect with SOS+causal architecture. hyp_006 switched to Apple's output-shift architecture, which eliminates log-det exploitation entirely (log_det/dof bounded at 0.5-1.3 vs 7+ for SOS). Multi-molecule training now works without exploitation, but VF remains at 13-25% — a normalization or training dynamics problem, not an architectural one.
...
- **[NEW — post-hyp_006]** VF plateau at 13-25% despite bounded log_det. Generated samples consistently have atom overlaps. Possible causes: (1) global normalization (dividing all coordinates by 1.29 Å global_std) amplifies small prediction errors into real overlaps during inverse normalization; (2) training budget may be insufficient (best checkpoint always at step 1000, but hyp_004 showed 44% VF on single ethanol at 20k steps); (3) multi-molecule mixing may require molecule-specific normalization. SCALE (9.6M params) overfits and performs WORSE than baseline — not a capacity issue.
