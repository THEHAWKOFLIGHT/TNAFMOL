...
- **[PARTIALLY RESOLVED — hyp_007]** VF plateau at 13-25% was caused by log-det exploitation re-emerging in multi-molecule training without regularization. Adding ldr=5.0 pushes ethanol VF from 17.6% → 55.8% and mean VF to 34.7%. The remaining gap (34.7% vs 98% per-molecule) correlates strongly with molecule size: small molecules (9 atoms) achieve 50%+, large molecules (18-21 atoms) achieve 10-25%. Possible causes: (1) d_model=128 capacity insufficient for 21-atom molecules in shared training; (2) cosine LR over-decays (best checkpoint at step 12000/20000, model stagnates after); (3) global normalization may still amplify errors for larger molecules.
...
11. **hyp_010**: TarFlow Apple Architecture for Multi-Molecule MD17 -- use tarflow_apple.py + TarFlow1DMol (proven 96-98% VF in und_001) directly for multi-molecule joint training. Bypasses model.py entirely. New training script (src/train_apple.py) adapts the und_001 architecture for MultiMoleculeDataset. Three-phase validation: (1) per-molecule sanity check, (2) padding validation, (3) multi-molecule OPTIMIZE. Goal: VF >= 50% on >= 4/8 molecules.
12. **hyp_011**: DDPM -- implement the diffusion baseline with comparable architecture, train and tune (OPTIMIZE).
13. **hyp_012** (conditional): Head-to-head comparison -- TarFlow vs DDPM on all metrics.
