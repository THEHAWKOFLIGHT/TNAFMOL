...
7. **hyp_006** (next — user decision needed): Options: (a) Test alpha_pos=0.02 + reg_weight=5 + Config D in multi-molecule, (b) Switch to Apple's output-shift architecture for multi-molecule, (c) Per-molecule TarFlow (T=n_real) for DDPM comparison — bypass padding problem.
