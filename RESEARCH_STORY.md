...
- **[RESOLVED — hyp_010]** VF plateau at 13-25% (hyp_006) and molecule-size correlation (hyp_007, 34.7% mean, 2/8 above 50%) were caused by model.py's architectural deficiencies, not fundamental limitations. Using the Apple architecture directly (tarflow_apple.py + TarFlow1DMol) achieves 71.6% mean VF with ALL 8 molecules above 50% — including aspirin (21 atoms) at 67.4%. No log-det regularization needed (ldr=0). Two critical padding bugs fixed: (1) sampling noise at padding positions corrupted PermutationFlip chain, (2) attention key masking starved first real atom of context in PermutationFlip. Remaining gap vs per-molecule ceiling (71.6% vs 98.2%): expected from multi-task capacity sharing (ethanol drops from 93.6% single-mol to 64% multi-mol).
